...
class PropertyApiView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        front_image = request.FILES.get('cover_image')
        back_image = request.FILES.get('cover_image')
        if cover_image:
        ...
        if front_image:
            front_image_path = default_storage.save(
                'property_cover_images/' +
                  front_image.name, ContentFile(front_image.read())
                )
        if back_image:
            back_image_path = default_storage.save(
                'property_cover_images/' +
                  back_image.name, ContentFile(back_image.read())
                )
        data = {
            ...
            'front_image' : front_image_path if front_image else None,
            'back_image' : back_image_path if back_image else None,
    ...
    def delete(self, request, prop_id, *args, **kwargs):
        try:
            ...
            reservations = Reservation.objects.filter(property_id=prop_id)
            reservations.delete()

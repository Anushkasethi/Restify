...
const UserCommentsComp = ({ user_id }) => {
  ...
  useEffect(() => {
    if (user_id) {
      const fetchComments = async () => {
        try { 
          const response = await axios.get(`http://localhost:8000/comments/addUserComment/${user_id}/`, {
            headers: {
  ...
  }, [user_id, page, filterDate]);

  const handlePostComment = async () => {
    try {
      const response = await axios.post(`http://localhost:8000/comments/addUserComment/${user_id}/`, {
        content: newComment,
        rating: newRating,
      }, {
        headers: {
            ...
            {false ? (
                ...
                comments.map((comment) => (
                    <div key={comment.id} className="comment">
                        ...
                        {comment.hostName}: {comment.content} (Rating: {comment.rating})
                        </Typography>
                    </div>
            ))
            )}
